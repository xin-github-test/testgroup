...
//5.对于需要改写的方法，写自己的代码
public class MyConnection implements Connection { //第一步
  private Connection oldConnection;//第二步  此时的oldConnection是com.mysql.jdbc.Connection
  private LinkedList<Connection> pool;//连接池对象

    public MyConnection(Connection oldConnection,LinkedList<Connection> pool)
    {
        this.oldConnection=oldConnection;//第三步，将实现Connetion接口的mysql的类赋值给上面的变量
        this.pool=pool;//得到连接池对象
    }

    @Override
    public void close() throws SQLException { //第五步，对于需要改变的方法，写自己的代码
     pool.addLast(oldConnection);
    }
    @Override
    public PreparedStatement prepareStatement(String sql) throws SQLException {
        return oldConnection.prepareStatement(sql);//第四步，对于不需要改写的方法调用原方法
    }
